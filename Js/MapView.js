...
let viewAll = () => {
  fetch("http://localhost:8080/map/viewAll").then((response) => {
...
let viewById = () => {
  ...
  fetch(`http://localhost:8080/map/viewById/${idValue}`).then((response) => {
...
let makeCard = (map) => {
  ...
  p.textContent = `Map ID: ${map.id} Map Name: ${map.name}`;
